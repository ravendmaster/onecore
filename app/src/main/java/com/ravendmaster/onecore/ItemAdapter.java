...
public class ItemAdapter extends DragItemAdapter<Pair<Long, WidgetData>, ItemAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        ...
        String pathPart=appSettings.username + "/" + presenter.getDevices().getDeviceMacByID(widget.deviceId) + "/out/";

        for(int i=0;i<4;i++) {
            values[i] = presenter.getMQTTCurrentValue(pathPart + widget.getTopic(i) + topic_suffix);
        }
