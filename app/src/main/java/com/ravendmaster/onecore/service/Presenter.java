...
import com.ravendmaster.onecore.DevicesCollection;
...
//            file = "help_application_server_mode.html";
    ...
    public void addNewDevice(String mac, String name) {
        int freeId = MainActivity.presenter.getFreeDevicesId();
    ...
    public void saveDevicesList(Context context) {
        AppSettings appSettings = AppSettings.getInstance();
        appSettings.saveDevicesSettingsToPrefs(context);
    }

    public String getRootTopicPathForSend(WidgetData widgetData) {
        return mqttService.getRootTopicPathForSend(widgetData);
    }

    public String getRootTopicPathForReceive(WidgetData widgetData) {
        return mqttService.getRootTopicPathForReceive(widgetData);
    }
    ...
    public int getFreeDevicesId() {
        return getDevices().getFreeId();
    }
    ...
    public void publicProgram(String mac, String program) {
        AppSettings appSettings = AppSettings.getInstance();
        publishMQTTMessage(appSettings.username + "/" + mac + "/in/prog", new Buffer(program.getBytes()), false);
    }
    ...
    public void onProgressChanged(SeekBar seekBar) {
        ...
        pack.topic = getRootTopicPathForSend(widget) + widget.getTopic(0);
        ...
        //setCurrentMQTTValue(widget.topic, currentInteractiveValue);
    ...
    public void onStopTrackingTouch(SeekBar seekBar) {
        ...
        String valueForSend = getSeekDisplayValue(widget, seekBar);
        if (!valueForSend.equals(mqttService.getCurrentMQTTValues().get(widget.getTopic(0)))) {
            //       publishMQTTMessage(widget.topic, valueForSend, true);
        }
    ...
    public void onMyButtonDown(MyButton button) {
        ...
        if (!widget.publishValue.equals("")) {
            widget.noUpdate = true;
            publishMQTTMessage(getRootTopicPathForSend(widget) + widget.getTopic(0), new Buffer(widget.publishValue.getBytes()), widget.retained);
        }
    }
    ...
    public void onMyButtonUp(MyButton button) {
        ...
        if (!widget.publishValue2.equals("")) {
            publishMQTTMessage(getRootTopicPathForSend(widget) + widget.getTopic(0), new Buffer(widget.publishValue2.getBytes()), widget.retained);
            //setCurrentMQTTValue(widget.topic, widget.publishValue2);//это костыль, чтобы связанные элементы не дергались
        }
        interactiveMode = false;
        view.onRefreshDashboard();
    }
    ...
    public void OnButtonsSetPressed(ButtonsSet buttonsSet, int index) {
        ...
        if (!widget.publishValue.equals("")) {
            //widget.noUpdate=true;
            //String []values=widget.publishValue.split(",");
            publishMQTTMessage(getRootTopicPathForSend(widget) + widget.getTopic(0), new Buffer(buttonsSet.getPublishValueByButtonIndex(index).getBytes()), widget.retained);
        }
        view.onRefreshDashboard();
    }
    ...
    public void sendMessageNewValue(String newValue) {
        publishMQTTMessage(getRootTopicPathForSend(widgetDataOfNewValueSender) + widgetDataOfNewValueSender.newValueTopic, new Buffer(newValue.getBytes()), false);
        //setCurrentMQTTValue(widgetDataOfNewValueSender.topic, "*" + newValue);
    }
    ...
    public void sendComboBoxNewValue(String newValue) {
        publishMQTTMessage(getRootTopicPathForSend(widgetDataOfNewValueSender) + widgetDataOfNewValueSender.topics[0], new Buffer(newValue.getBytes()), widgetDataOfNewValueSender.retained);
    }


    public boolean isAdfree() {
        return true;
        //return mqttService.isAdfree();
    }
