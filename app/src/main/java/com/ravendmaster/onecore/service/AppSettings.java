...
public class AppSettings {
    ...
    public void addDevice(Device device) {
        devices.getItems().add(device);
    }

    public String[] getDeviceNames() {
        String[] result = new String[devices.getItems().size()];
        int index = 0;
    ...
    public Device getDeviceByMac(String mac){
        for (Device device:devices.getItems()) {
            if(device.mac.equals(mac))return device;
        }
        return null;
    }

    public Device getDeviceByID(int id){
        Device res=getDeviceByMac(getDeviceMacById(id));
        return res==null?new Device():res;
    }

    public int getDevicesIdByIndex(int index) {
        return devices.getItems().get(index).id;
    }
    ...
    public void readFromPrefs(Context con) {
        ...
        */
        ...
        sprefs = con.getSharedPreferences("mydevices", Context.MODE_PRIVATE);
        String tabsJSON = sprefs.getString("devices", "");
        devices.setFromJSONString(tabsJSON);
        /*
        sprefs = con.getSharedPreferences("dashboard", Context.MODE_PRIVATE);
        int i = 0;
        for (TabData tabData : tabs.getItems()) {
    ...
    public void saveDevicesSettingsToPrefs(Context con) {
        SharedPreferences sprefs = con.getSharedPreferences("mydevices", Context.MODE_PRIVATE);
        SharedPreferences.Editor ed = sprefs.edit();

        ed.putString("devices", devices.getAsJSONString());
        if (!ed.commit()) {
            Log.d(getClass().getName(), "commit failure!!!");
        }
    }
    ...
    public String getSettingsAsString() {
        ...
        try {
            ...
            resultJson.put("devices", devices.getAsJSONString());
        ...
        try {
            ...
            while (jsonReader.hasNext()) {
                ...
                switch (name) {
                    ...
                    case "devices":
                        devices.setFromJSONString(jsonReader.nextString());
                        break;
